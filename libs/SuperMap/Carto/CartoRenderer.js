...
SuperMap.CartoRenderer = SuperMap.Class({
    ...
    drawGeometry:function(ctx,geometry, style,hitColor) {
        ...
        if(hitColor){
            this.hitContext.fillStyle=hitColor;
            this.hitContext.strokeStyle=hitColor;
            this.hitContext.lineWidth=ctx.lineWidth+3;
        ...
        switch(type){
            ...
            case "LINE":
                ...
                drawed = this.drawLine(ctx,geometry, style);
                break;
            case "REGION":
                drawed = this.drawRegion(ctx,geometry, style);
                break;
            default:break;
        }
        //绘制后恢复绘制前的canvas的状态
     ...
     * y - 点的y坐标
     * */
    drawImagePoint:function(ctx,style,x,y){
     ...
     * style - {Object} 渲染风格
     * */
    drawLine:function(ctx,geometry, style,isRegion){
        ...
        for (var i = 0,len=geometry.parts.length; i < len; i++) {
            ...
            if(style.lineDasharray&&style.lineDasharray.length>0){
               if(ctx.setLineDash){
                   ...
                   this.drawDotedLine(ctx,points,startIndex,part,style,isRegion,noline);
                   ctx.setLineDash([]);
               }else{
                   if(!noline){
                       ...
                       this._drawDoteLine(ctx,points,startIndex,part,style,lineDasharray,lineDashOffset);
                   }
                   if(isRegion){
                       if(style.polygonOpacity){
                           ctx.globalAlpha=style.polygonOpacity;
                       }
                       this.drawDotedLine(ctx,points,startIndex,part,style,true,true);
                   }
               }
            }else{
                this.drawDotedLine(ctx,points,startIndex,part,style,isRegion,noline);
     ...
     * offset -线的偏移值
     * */
    drawDotedLine:function(ctx,points,startIndex,length,style,isRegion,noLine){
     ...
     * ie9及ie10下使用的绘制虚线算法
     * */
    _drawDoteLine:function(ctx,points,startIndex,length,style,lineDasharray,lineDashOffset){
     ...
     * style - {Object} 渲染风格
    * */
    drawRegion:function(ctx,geometry, style){
        return this.drawLine(ctx,geometry,style,true);
    },
