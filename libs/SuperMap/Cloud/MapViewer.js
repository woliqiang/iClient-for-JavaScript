...
SuperMap.Cloud.MapViewer=SuperMap.Class({
     ...
     * {Array} 事件数据
     * */
    EVENT_TYPES: ["baselayerInitialized","layersInitialized","markerClicked","featureSelected","featureUnSelected","featureEditing","featureEdited"],
     ...
     * options - {Object} 可选参数，用于批量设置MapViewer对象的相应属性
     * */
     ...
     * */
    previewMapById:function(mapid,container){
        var url=this.url + mapid +".json";
        ...
        SuperMap.Util.committer({
            ...
            success:function(isInTheSameDomain){
                return function(result){
                    ...
                    this.previewMapByJson(jsonObj,container);
                }
            }(isInTheSameDomain),
     ...
     * container - {String|DOMElement}  [可选参数]地图容器DOM元素或者其id。
     * */
    previewMapByJson:function(jsonObj,container){
        if(!jsonObj){
            return;
        }
        //先清除上一次创建的map
        ...
        for(var i= 0;i<len;i++){
            ...
            if(type.indexOf("SUPERMAP_REST")>-1){
                layer=this.asynCreateLayer(type,layerInfo,function(layer){
                    ...
                    this.addBaseLayer2Map(map,layer,center,level,bounds,layerQueue);
                },this);
            }else{
    ...
    createTiledLayer:function(layerInfo,callback,scope){
        var tiledLayer,
            ...
            epsgCode=this.mapInfo.epsgCode||layerInfo.epsgCode,
            url=layerInfo.url,
    ...
    createWmtsLayer: function (layerInfo) {
        var title=layerInfo.title,
            ...
            epsg=this.mapInfo.epsgCode||layerInfo.epsgCode;
