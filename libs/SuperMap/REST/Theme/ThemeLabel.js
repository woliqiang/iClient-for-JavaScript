...
SuperMap.REST.ThemeLabel.fromObj = function(obj) {
    if(!obj) return;
    var lab = new SuperMap.REST.ThemeLabel();
    var itemsL = obj.items, cells = obj.matrixCells;
    obj.matrixCells = null;
    SuperMap.Util.copy(lab, obj);
    lab.alongLine = SuperMap.REST.ThemeLabelAlongLine.fromObj(obj);
    lab.background = SuperMap.REST.ThemeLabelBackground.fromObj(obj);
    lab.flow = new SuperMap.REST.ThemeFlow({flowEnabled: obj.flowEnabled,leaderLineDisplayed: obj.leaderLineDisplayed,leaderLineStyle: obj.leaderLineStyle});
    if(itemsL) {
        lab.items = [];
        for(var i = 0, len = itemsL.length; i < len; i++ ) {
            lab.items.push(SuperMap.REST.ThemeLabelItem.fromObj(itemsL[i]));
        }
    }
    if(cells) {
        lab.matrixCells = [];
        for(var i = 0, len = cells.length; i < len; i++) {
            //TODO
            //lab.matrixCells.push(SuperMap.REST.LabelMatrixCell.fromObj(cells[i])); 
        }
    }
    lab.offset = SuperMap.REST.ThemeOffset.fromObj(obj);
    lab.text = SuperMap.REST.ThemeLabelText.fromObj(obj);
    return lab;
}
